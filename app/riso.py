...
.riso-strip {{ display: flex; justify-content: space-between; flex-wrap: wrap; gap: 6px 16px; border-bottom: 2px solid {INK}; padding-bottom: 10px; }}
...
.riso-title {{ position: relative; margin: 18px 0 26px; font-family: "Anybody", Impact, sans-serif; font-weight: 900; font-stretch: 150%;
  text-transform: uppercase; font-size: clamp(38px, 7vw, 84px); line-height: .86; letter-spacing: -.01em; text-wrap: balance; color: {INK}; }}
...
.riso-title .ghost {{ position: absolute; inset: 0; transform: translate(4px, 3px); mix-blend-mode: multiply; pointer-events: none; }}
...
.riso-stamp {{ display: inline-block; font-family: "Anybody", Impact, sans-serif; font-weight: 800; font-stretch: 125%; text-transform: uppercase;
  font-size: clamp(22px, 2.8vw, 30px); color: {PAPER}; padding: 7px 16px 5px; mix-blend-mode: multiply; }}
...
.riso-facts dd {{ margin: 0; font-family: "Anybody", Impact, sans-serif; font-weight: 700; font-stretch: 110%; font-size: 25px;
  font-variant-numeric: tabular-nums; border-bottom: 1px dashed {INK}66; padding-bottom: 8px; }}
...
.riso-foot {{ border-top: 1px dashed {INK}66; padding-top: 12px; display: flex; flex-wrap: wrap; justify-content: space-between; gap: 8px 20px; }}
...
def _chart_svg(page, episodes, revealed, frame, css_class):
    ...
    for tick in range(lo, hi + 1):
        parts.append(f'<line x1="{L}" x2="{W - R}" y1="{y(tick):.1f}" y2="{y(tick):.1f}" stroke="{INK}" stroke-opacity=".13"/>')
    ...
    for season, a, b in seasons:
        if b < n - 1:
            sx = (x(b) + x(b + 1)) / 2
            parts.append(f'<line x1="{sx:.1f}" x2="{sx:.1f}" y1="{T}" y2="{H - B}" stroke="{INK}" stroke-opacity=".3" stroke-dasharray="3 4"/>')
