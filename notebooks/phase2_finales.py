...
print(f"  finale beats its season body: {100*(sfp>0).mean():.1f}%")
print(f"  finale peaks by >= +0.5:      {100*(sfp>=0.5).mean():.1f}%")
print()
print("=" * 60)
...
print(f"  premiere beats its season body: {100*(spp>0).mean():.1f}%")
print()
...
print(f"  last episode rises:        {100*(sr['finale_effect']>0).mean():.1f}%")
print(f"  last episode gains >= 0.5: {100*(sr['finale_effect']>=0.5).mean():.1f}%")
print(f"  last episode drops <= 0.5: {100*(sr['finale_effect']<=-0.5).mean():.1f}%")
print()
...
out = {
    "season_finale_premium": {"mean": round(float(sfp.mean()), 3), "median": round(float(np.median(sfp)), 3),
                              "pct_above": round(100*float((sfp > 0).mean()), 1)},
    "season_premiere_premium": {"mean": round(float(spp.mean()), 3), "pct_above": round(100*float((spp > 0).mean()), 1)},
    "series_finale_effect": {"mean": round(float(sr["finale_effect"].mean()), 3),
                             "pct_up": round(100*float((sr["finale_effect"] > 0).mean()), 1),
                             "pct_flop": round(100*float((sr["finale_effect"] <= -0.5).mean()), 1)},
}
