...
def test_every_show_renders_in_both_states(every_show):
    for page, episodes, context in every_show:
        for revealed in (False, True):
            riso.masthead(page, revealed)
            riso.chart(page, episodes, revealed)
        riso.verdict(page, episodes, context, guess=None)
        riso.context_block(page, context)


def test_nothing_before_the_reveal_gives_the_verdict_away(every_show):
    """The guess is only worth making if the page has not already answered it:
    no verdict ink, no verdict word, no trend line until the reveal."""
    for page, episodes, _ in every_show:
        before = riso.masthead(page, revealed=False) + riso.chart(page, episodes, revealed=False)
        ...
        assert 'stroke-width="6"' not in before, page["title"]
...
def test_season_labels_never_collide(every_show):
    ...
    for page, episodes, _ in every_show:
        for frame in (riso.WIDE, riso.NARROW):
            svg = riso._chart_svg(page, episodes, True, frame, "x")
